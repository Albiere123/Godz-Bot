...
exports.run = (client, message, args) => {
 // Agora, vamos criar uma embed para explicar o uso do comando
  ...
  .addField(`:book: **Exemplo**`, `\`${c.prefix}addemoji Faker https://discordemoji.com\``)
...
try { // Utilizando a função 'try', literalmente traduzindo: Tentar
    message.guild.emojis.create(args[1], args[0]).then(emoji => { // Criar um emoji com as informações
      message.channel.send(`${emoji} **|** Emoji adicionado com sucesso. Seu id é ${emoji.id}`); // Caso não haja erro, iremos criar
    });
...
exports.help = {
  name: 'addemoji',
  aliases: ['criaremoji', 'adicionar emoji']
}