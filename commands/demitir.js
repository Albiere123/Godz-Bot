...
exports.run = async (client, message, args) => {
  ...
  if (conta < 3500) return message.reply('para pedir demissão, você necessita de **R$ 3500**!') // caso o usuário não possua, daremos o erro
...
if (emprego === null) { // caso o 'trabaio' seja null, ou seja, sem, iremos avisar que ele precisa ter um emprego
  ...
  if (emprego === 1) { // No arquivo para escolher um trabalho, definimos que 1 é igual à um Programador... Então, vamos fazer uma mensagem bonita
    message.channel.send(`Olá **${message.author.username}**, você realmente deseja largar a vida de Programador? Saiba: Você pagará **R$ 3500**`).then(msg => {
    ...
    msg.awaitReactions(filter, {max: 1}) // retornando com a reação que puxamos acima
      ...
      .then(collected => { // mais uma função 'then', nomeada de 'collected' para definirmos que foi coletado
        ...
        if (reaction.emoji.name === '✅') { // caso o usuário clique no emoji referente à correto, iremos fazer a ação
          ...
          db.subtract(`money_${message.author.id}`, 3500) // removendo do 'money' da database
  ...
  if (emprego === 2) { // agora, o emprego dois, que é o Designer
    message.channel.send(`Olá **${message.author.username}**, você realmente deseja sair do mundo Designer? Saiba: Você terá que pagar **R$ 3500**`).then(msg => {
    ...
    msg.awaitReactions(filter, {max: 1}) // retornando com as reações
      .then(collected => { // e mais uma função then, que vai ser como o collected acima
        ...
        if (reaction.emoji.name === '✅') { // caso o usuário clique no emoji referente à correto, iremos fazer a ação
          ...
          db.subtract(`money_${message.author.id}`, 3500) // removendo do 'money' na database
